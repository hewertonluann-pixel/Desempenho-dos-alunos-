...
const fotoFile = document.getElementById("novoFoto")?.files
;
...
<input type="file" id="foto-${aluno.id}" accept="image/*" style="display:none;" onchange="atualizarFoto('${aluno.id}', this.files
)">
</div>
...
// Inserir estilos se não existirem
...
if (btnModoAluno) btnModoAluno.onclick = () => { if (user.nome) window.location.href = aluno.html?nome=${encodeURIComponent(user.nome)}; };
...
finally { if (btn) { btn.disabled = false; btn.textContent = "⚡ Atualizar Comprometimento"; } }
}
